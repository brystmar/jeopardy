...
Requires data stored in two temp tables:
    ...
    "wager"     (game_id, crtid, wager) [for DD wagers]
"""
...
def write_cr(cr):
    if cr['player_id'] == None:
        jdata.execute('SELECT id FROM clue_response WHERE clue_id = ? and player_id is null', (cr['clue_id'],))
    ...
    if val == None:
        jdata.execute('''insert INTO clue_response (clue_id, player_id, response, correct, order_within_clue, end_of_round, player_score_impact, notes, unsure, game_id, crtid)
...
while loop_counter < total_loops: #for each chunk of games retrieved...
    ...
    jdata.execute('''SELECT distinct d.game_id, c.id, d.crtid, d.html, d.cr_notes, c.correct_response, g.clues_before_break FROM clue_div d JOIN clue c on d.crtid = c.crtid and d.game_id = c.game_id JOIN game g on c.game_id = g.id
        WHERE d.game_id >= ? and d.game_id <= ? order by 1,2,3''', (gmin, gmax))
        # d.crtid = 'clue_TB' order by 1,2,3')
    data = jdata.fetchall()

    if data == None or len(data) == 0:
        # no data for any of the games retrieved
        while gmin <= gmax:
            write_errors(gmin, None, "Game has no clue data")
            missing(gmin, None, None, "Game", "Partial", "Game has no clue data")
            print("No data for game", gmin)
            gmin += 1
        i += 1
        gmin = gmax + 1
        game_id = gmin
    ...
    for d in data: #for every clue within those games...
        if d[2] == None: #crtid should never be null
        ...
        crtid = d[2]
        html = d[3]
        banter = remove_html_tags(d[4])
        correct_response = remove_html_tags(d[5])
        cbb = d[6]

"""
Find & fix nickname disparities between game_player and player_round_result
"""
...
jdata.execute("""select distinct prr.player_id as prr_player, prr.nickname as prr_nick, gp.player_id as gp_player, gp.nickname as gp_nick
    FROM player_round_result as prr JOIN game_player as gp on prr_player = gp_player
    WHERE gp_nick is null ORDER BY 1,2,3""")
data = jdata.fetchall()

for d in data:
    print(d[0], d[1], d[2])
    jdata.execute("UPDATE game_player SET nickname = ? WHERE player_id = ?", (d[1], d[2]))
    commit(conn_jdata)
    i += 1

print("")
print(i, "updates made")
print("")
